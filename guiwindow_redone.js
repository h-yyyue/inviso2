...
import Config from '/js/data/config';
...
export default class GUIWindow {
  ...
  initObjectGUI(object) {
      ...
      this.addParameter({
          property: 'Controls',
          value: object.omniSphere.sound ? this.convertPlayPause(object.omniSphere.sound.state.isAudioPaused) : this.convertPlayPause(true),
          type: 'controls',
          cls: 'controls',
          bind: audioPlayPause
      }, elem);
  ...
  addCone(cone) {
    this.undoableActionStack.push(new Action(this.obj, 'addCone'));
    this.undoableActionStack[this.undoableActionStack.length - 1].secondary = cone;
    ...
    this.addParameter({
        property: 'Controls',
        value: this.convertPlayPause(cone.sound.state.isAudioPaused),
        type: 'controls',
        cls: 'controls',
        bind: audioPlayPause
    }, elem);
  ...
  updateObjectGUI(object) {
      ...
      var audioPlay = this.container.querySelector('#audio-controls');
      if (object.omniSphere.sound) {
        audioPlay.src = object.omniSphere.sound.state ? this.convertPlayPause(object.omniSphere.sound.state.isAudioPaused) : this.convertPlayPause(true);
      }
      ...
      if (object.cones && object.cones.length > 0) {
        ...
        object.cones.forEach((cone, i) => {
          if (cone === this.app.interactiveCone) {
            ...
            this.replaceTextContent(cones[i].querySelector('#audio-controls'), this.convertPlayPause(cone.sound.state.isAudioPaused));
          }
          else {
  ...
  detachSound(fileSpan, removeSpan) {
    ...
    if (this.obj.type === "SoundObject") {
      if (this.obj.omniSphere.sound && this.obj.omniSphere.sound.source) {
        ...
        this.obj.omniSphere.sound.state.clear();
  ...
  audioPlayPause(e, type) {
    var obj = this.obj;
    var span = e.target;
    if (type === 'omnisphere') {
      if (obj.omniSphere.sound) {
        if (obj.omniSphere.sound.state.isAudioPaused) {
          this.startPlaying(type);
        }
        else {
          this.stopPlaying(type);
        }
      }         
    }
    else if (type === 'cone') {
        if (cone.sound.state.isAudioPaused) {
          this.startPlaying(type, cone);
        }
        else {
          this.stopPlaying(type, cone);
        }
    }  

    
    // var button = this.container.querySelector('#audio-controls');
  ...
  startPlaying(type, cone) {
    if (type === 'omnisphere') {
      this.obj.playSound(type)
    }
    else if (type === 'cone') {
      this.obj.playSound(type, cone);
    }
  ...
  stopPlaying(type, cone) {
    if (type === 'omnisphere') {
      this.obj.stopSound(type)
    }
    else if (type === 'cone') {
      this.obj.stopSound(type, cone);
    }
    else if (obj.type === 'SoundZone') {
  ...
  convertPlayPause(isAudioPaused) {
    if (isAudioPaused) {
      return "../../assets/models/play.png";
    }
    return "../../assets/models/pause.png";
  }
  ...
  startDragging(e) {
    ...
    if (l && l.callback) {
      ...
      this.dragEvent.callbackName = l.callback.name
      this.dragEvent.editing = e.target;
      this.dragEvent.x = e.x;

      // if (this.dragEvent.callbackName === 'changeAudioTimeOmniSphere') {
      //   this.stopPlaying("omnisphere");
      // }
      // else if (this.dragEvent.callbackName === 'changeAudioTimeCone') {
      //   this.stopPlaying("cone");
      // }
    }
  }
  drag(e) {
...
addSwipeEvents(div, title, isObject) {
  ...
  addParameter(p, container) {
      ...
      if (p.bind) {
        ...
        val.onkeydown = this.typingInput.bind(this);
        if (p.type === 'controls') { val.onclick = this.changeAudioPlay.bind(this); }
        this.listeners.push({
          elem: val,
          callback: p.bind
        })
      ...
      if (p.type === 'number') {
        ...
        span.after(document.createTextNode(" min" + '\u00A0\u00A0'));
        ...
        // audioBtn.onclick = this.audioPlayPause.bind(this, p.bind.name);

        // btnVal.appendChild(audioBtn);
      }
      else if (p.type === 'controls') {
        var audioBtn = document.createElement('img');
        val.appendChild(audioBtn);

        audioBtn.className = 'value';
        audioBtn.id = "audio-controls";
        audioBtn.style.height = "12px";
        audioBtn.style.width = "12px";
        audioBtn.style.opacity = "0.7";
      ...
      // if (p.type == 'time') { div.appendChild(btnVal); }
